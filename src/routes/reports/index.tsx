...
function ReportsPage() {
  const { data: sessions = [] } = useQuery<ReportSessionMeta[]>({
    queryKey: ["report-sessions"],
    queryFn: fetchReportSessions,
  });
      ...
      {sessions.length === 0 ? (
        ...
        >
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            ...
            <tbody>
              {sessions.map((s) => (
                <tr key={s.session_id} style={{ borderTop: `1px solid ${T.border}` }}>
                  <td style={{ padding: "10px 16px" }}>
                    <TypeBadge type={s.session_type} />
                  </td>
                  <td style={{ padding: "10px 16px", color: T.text }}>{s.algo_name ?? "—"}</td>
                  <td style={{ padding: "10px 16px" }}>
                    <Link
